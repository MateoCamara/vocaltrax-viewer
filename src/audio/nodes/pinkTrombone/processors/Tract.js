/*
    tract_simplified_final.js

    VERSIÓN FINAL CON CORRECCIÓN DE EXPORTACIÓN:
    Se ha añadido `export default` a la definición de la clase. Esto soluciona
    los errores de importación que impedían que la aplicación se cargara.
*/
...
export default class TractSimplifiedFinal {
// ===================================================================

    constructor(length = 44) {
        // ... (el resto del constructor es idéntico al de la versión anterior y funcional)
        this.length = length;
        this.blade = { start : Math.floor(10 * this.length/44) };
        this.tip = { start : Math.floor(32 * this.length/44) };
        this.lip = { start : Math.floor(39 * this.length/44), reflection : -0.85 };
        ...
        this.tongue = {
            _diameter : 2.43, _index : 12.9,
            range : {
                diameter : {
                    minValue : 2.05, maxValue : 3.5,
                ...
                index : {
                    minValue : this.blade.start+2, maxValue : this.tip.start-3,
                    get range() { return this.maxValue - this.minValue; },
                    get center() { return (this.maxValue + this.minValue)/2; },
                    centerOffset(interpolation) {
                        const centerOffsetDiameter = interpolation * this.range;
                        return centerOffsetDiameter/2;
                    },
                },
            },
            get diameter() { return this._diameter; },
            set diameter(newValue) { this._diameter = Math.clamp(newValue, this.range.diameter.minValue, this.range.diameter.maxValue); },
    ...
    // Simplemente cópialo y pégalo aquí.
    ...
    _processNose(parameterSamples, bufferInterpolation, updateAmplitudes) {
        ...
        for(let index = 0; index < this.nose.length; index++) {
            ...
            if(updateAmplitudes) {
                ...
                this.nose.amplitude.max[index] = (sum > this.nose.amplitude.max[index])? sum : this.nose.amplitude.max[index] * 0.999;
            }
        }
    ...
    _updateDiameterRest() {
        for(let index = this.blade.start; index < this.lip.start; index++) {
            ...
            this.diameter.rest[index] = 1.5 - curve;
        }
    }

    _updateConstrictions(constrictions) {
        this._updateDiameterRest();
        for(let index = 0; index < this.length; index++) this.diameter[index] = this.diameter.rest[index];
        this.velum.target = 0.01;
        for(let index = -1; index < constrictions.length; index++) {
            ...
            if(constriction.index >= 2 && constriction.index < this.length && constriction.diameter > -(0.85 + this.nose.offset)) {
                var newTractDiameter = constriction.diameter - 0.3;
                ...
                if(newTractDiameter < 3) {
                    ...
                    else tractIndexRange = 10 - 5 * (constriction.index-25) / (this.tip.start - 25);
                    ...
                    for(let tractIndex = constrictionIndex - constrictionIndexRadius; (tractIndex < constrictionIndex + tractIndexRange+1) && (tractIndex >= 0 && tractIndex < this.length); tractIndex++) {
                        const tractIndexOffset = Math.abs(tractIndex - constriction.index) - 0.5;
                        var tractDiameterScalar;
                        ...
                        if(this.diameter[tractIndex] > newTractDiameter) {
                            this.diameter[tractIndex] = newTractDiameter + (this.diameter[tractIndex] - newTractDiameter) * tractDiameterScalar;
                        }
                    }
                }
            }
        ...
        this.previousConstrictions.tongue = { index : this.tongue.index, diameter : this.tongue.diameter };
    }
    ...
    reset() {
        this.right.fill(0); this.right.junction.fill(0);
        this.left.fill(0); this.left.junction.fill(0);
        if (this.nose) { this.nose.left.fill(0); this.nose.left.junction.fill(0); this.nose.right.fill(0); this.nose.right.junction.fill(0); }
    }
}